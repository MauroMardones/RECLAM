...
archivos <- list(
  ...
  marzo = "DATA/Chirla 07_04_2025 (MARZO).xlsx",
  abril = "DATA/Chirla 23_04_2025.xlsx",
  mayo = "DATA/Chirla 20_05_2025.xlsx",
  junio = "DATA/Chirla 02_07_2025 (junio).xlsx"
  #julio2 = "DATA/Chirla 25_07_2025.xlsx",
  #agosto2 = "DATA/Chirla 09_08_2025.xlsx"
  # marzo hasta junio
)
...
archivosco <- list(
  ...
  marzo = "DATA/Coquina 14_03_2025.xlsx",
  abril = "DATA/Coquina 25_04_2025.xlsx",
  mayo = "DATA/Coquina 27_05_2025.xlsx",
  junio = "DATA/Coquina 26_06_2025.xlsx",
  julio2 = "DATA/Coquina 24_07_2025.xlsx",
  agosto2 = "DATA/Coquina 26_08_2025.xlsx",
  septiembre2 = "DATA/Coquina 23_09_2025.xlsx"
  # marzo hasta junio
)
# Leer cada archivo y almacenarlo en una lista
...
total_c <- bind_rows(datos_totales, datos_totalesco) %>%
  ...
  rename(REPLICA = hoja)
saveRDS(total_c,  "DATA/Base_RECLAM_Tallas_Cadiz.rds")
write_csv(total_c, "DATA/Base_RECLAM_Tallas_Cadiz.csv")
```
...
total_c %>%
  group_by(REPLICA, especie, MES, ANO) %>%
  summarise(n = n(), .groups = "drop") %>%
  ggplot(aes(x = REPLICA, y = n, fill = as.factor(especie))) +
...
```{r}
Bitácora_chirla<- read_excel("DATA/Bitácora_chirla_RECLAM.xlsx")

# lo mismo para coquina
Bitácora_coquina<- read_excel("DATA/Bitácora_coquina_RECLAM.xlsx")

# ---- CHIRLA (preprocesamiento estructural) ----
chirla_std <- Bitácora_chirla %>%
  mutate(
    Réplica = as.character(Punto),
    Punto = str_extract(Réplica, "^[0-9]+")
  ) %>%
  transmute(
    Especie = "CH",
    Punto = as.character(Punto),
    `Réplica` = as.character(Réplica),
    Mariscador = NA_character_,
    Latitud = as.numeric(`Latitud inicio`),
    Longitud = as.numeric(`Longitud inicio`),
    Track = as.character(Track),
    Fecha = as.POSIXct(Fecha),
    `Temperatura agua` = NA_character_,
    `Tiempo de pesca(minutos)` = as.numeric(difftime(`Hora Fin`, `Hora Inicio`, units = "mins")),
    `Distancia (m)` = as.numeric(`Recorrido(m)`),
    `Hora de llegada` = `Hora Fin`,
    Observaciones = as.character(Observaciones)
  )

# ---- COQUINA ----
coquina_std <- Bitácora_coquina %>%
  transmute(
    Especie = "CO",
    Punto = as.character(Punto),
    `Réplica` = as.character(`Réplica`),
    Mariscador = as.character(Mariscador),
    Latitud = as.numeric(Latitud),
    Longitud = as.numeric(Longitud),
    Track = as.character(Track),
    Fecha = as.POSIXct(Fecha),
    `Temperatura agua` = as.character(`Temperatura agua`),
    `Tiempo de pesca(minutos)` = as.numeric(`Tiempo de pesca(minutos)`),
    `Distancia (m)` = as.numeric(`Distancia (m)`),
    `Hora de llegada` = `Hora de llegada`,
    Observaciones = as.character(Observaciones)
  )

# ---- UNIÓN FINAL ----
bitacora_total <- bind_rows(chirla_std, coquina_std)
#saveRDS(bitacora_total, "DATA/Base_RECLAM_Bitacora_Cadiz.rds")
#write_csv(bitacora_total, "DATA/Base_RECLAM_Bitacora_Cadiz.csv")
```


```{r}
Pesos_Chirla_RECLAM <- read_excel("DATA/Pesos Chirla_RECLAM.xlsx")
Pesos_coquina_RECLAM <- read_excel("DATA/Pesos Coquina_RECLAM.xlsx")
```
